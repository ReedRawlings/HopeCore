...
//
//  DUPLICATE LOCATION: HopeCoreWidget/WidgetDataStore.swift
//  Keep both files in sync if making changes!
//
...
struct WidgetDataStore {
    ...
    /// Save scheduled messages for today's widget updates
    /// Each message will be shown at its scheduled notification time
    /// - Parameter schedule: Array of tuples (time: Date, message: WidgetMessage)
    static func saveScheduledMessages(_ schedule: [(time: Date, message: WidgetMessage)]) {
        guard let defaults = sharedDefaults else {
            print("❌ WidgetDataStore: Cannot save schedule - App Group not accessible")
            return
        }
        
        // Convert to serializable format
        let scheduleData: [[String: Any]] = schedule.map { item in
            return [
                "time": item.time.timeIntervalSince1970,
                "message": [
                    "id": item.message.id.uuidString,
                    "text": item.message.text,
                    "categoryName": item.message.categoryName
                ]
            ]
        }
        
        defaults.set(scheduleData, forKey: scheduledMessagesKey)
        defaults.synchronize()
        
        print("✅ WidgetDataStore: Saved \(schedule.count) scheduled messages for widget")
    }
...
// MARK: - Widget Message (Lightweight Model)
